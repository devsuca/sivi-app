...
def log_visitante_save(sender, instance, created, **kwargs):
    user = get_current_user()
    if created:
        acao = "criacao"
        dados_novos = json.dumps(instance.__dict__, default=str)
        LogSistema.objects.create(entidade="Visitante", acao=acao, user=user, dados_novos=dados_novos)
    else:
        acao = "atualizacao"
...
def log_visitante_delete(sender, instance, **kwargs):
    user = get_current_user()
    acao = "exclusao"
    dados_anteriores = json.dumps(instance.__dict__, default=str)
    LogSistema.objects.create(entidade="Visitante", acao=acao, user=user, dados_anteriores=dados_anteriores)
