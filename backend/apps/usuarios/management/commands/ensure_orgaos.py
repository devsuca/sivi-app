...
class Command(BaseCommand):
    help = 'Garante que todos os usuários tenham órgão associado'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('🔧 Verificando órgãos dos usuários...')
        )
        ...
        users_with_orgao = Usuario.objects.filter(orgao__isnull=False).count()
        
        self.stdout.write(f'Total de usuários: {total_users}')
        self.stdout.write(f'Usuários com órgão: {users_with_orgao}')
        self.stdout.write(f'Usuários sem órgão: {total_users - users_with_orgao}')
        
        if total_users == users_with_orgao:
            self.stdout.write(self.style.SUCCESS('🎉 SUCESSO: Todos os usuários possuem órgão!'))
        else:
            self.stdout.write(self.style.ERROR('❌ AINDA HÁ USUÁRIOS SEM ÓRGÃO!'))
